...
- **Redis integration tests (JAV-37):** start `docker compose -f compose.redis.yml up -d --wait`, then `mvn -Predis-it verify`. Requires local Redis; uses unique test keys, real HTTP on random ports, mocked auth and no PostgreSQL. Default tests still need no external services.
...
- **Package layout (`src/main/java/com/kanban`):**
  ...
  - `modules/<feature>/` — one package per Nest module: entity, `XRepository` (Spring Data), `XService`, `XController`, `dto/`, optional `events/`. `modules/events` is the Socket.IO gateway (`EventsGateway`, `EventsService`, `WsJwtGuard`, `socket/NettySocketIoServer` behind `SocketServer`/`SocketClient` adapters). `modules/board` is read-only aggregation (no entity); `modules/dependency` is the task blocks/blocked-by graph (JSP-33; `task_dependencies` edges, recursive-CTE cycle check); `modules/mention` is service-only; `modules/search` is the cross-project task full-text search (`GET /search/tasks`, JAV-34; no entity) — its `TaskSearchSql` holds the one `tsvector` predicate, which the board filter reuses.
...
- **No global response envelope.** Three shapes coexist — match the module you're editing: list endpoints use `ApiListResponse.ok(list)` = `{ data, status, success, message? }`; single-entity/board endpoints return the raw `toJson()` map / response record; paginated comments and `GET /search/tasks` return `PaginatedResponse` = `{ data, meta: { page, limit, total, totalPages } }` — reuse that exact shape for new paginated lists.
...
- **Login rate limiting (JAV-37):** opt-in `RATE_LIMIT_ENABLED=true`; `@RateLimited("login")` is checked by `RateLimitInterceptor` before argument validation. `RedisRateLimiter` executes `redis/rate-limit.lua` atomically (quota + TTL); no local counters. Default 10/IP/60s, first request anchors the window. Denials return 429 + `Retry-After`, Redis failures generic 503. `RateLimitConfig` is absent when disabled. Client IP comes only from Tomcat's `getRemoteAddr()`; native forwarding trusts only `TRUSTED_PROXY_REGEX` (none by default). Do not parse arbitrary forwarded headers in controllers/interceptors. See `docs/api-contracts/login-rate-limit.md` and `docs/queries/auth.md`.
...
- **Access:** constructor-injected `XRepository extends JpaRepository<Entity, Id>`. Derived queries / `@Query` JPQL for what the repository API can express; a `*Queries` interface + `Jpa*Queries` `@Repository` impl (`EntityManager` native SQL) only for joins/aggregates it can't (`ProjectAccessQueries`, `BoardQueries`, `TaskSearchQueries`); `TaskPositionFunctions` (`JpaTaskPositionFunctions`) is the only place the position stored procedures are called.
...
- **Task search:** `tasks.search_vector` is a generated `tsvector` (config `simple`, title weight A / description B) with a GIN index (V4). Search tasks only through `TaskSearchSql` (`websearch_to_tsquery('simple', :search)` against `search_vector`) — never `ILIKE`, never another config (the parser config must match the column's). Cap free-text inputs with `@MaxLength(200)`: one ≥ 2047-byte token makes `websearch_to_tsquery` throw. Decisions, plans and snippet rules: `docs/queries/search.md`.
