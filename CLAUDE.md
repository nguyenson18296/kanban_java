...
- **Socket.IO runs on its own port (1997, netty-socketio)** — Tomcat cannot host the Socket.IO protocol. Event names, payloads, rooms (`user:<id>`, `project:<id>`) and the `token:refresh` flow match Nest; see `MIGRATION.md` §6.
